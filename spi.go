...
func New(bus byte, channel byte, mode byte, speed uint32) (*SPI, error) {
	...
	err = ret.setMode(mode)
	if err != nil {
		return nil, err
	}

	err = ret.setSpeed(speed)
	if err != nil {
		return nil, err
	}
...
func (spi *SPI) setMode(mode uint8) error {
	_, _, errno := syscall.Syscall(syscall.SYS_IOCTL, spi.rc.Fd(), spiIOCWrMode, uintptr(unsafe.Pointer(&mode)))
	if errno != 0 {
		err := syscall.Errno(errno)
		return err
	}
...
func (spi *SPI) setSpeed(speed uint32) error {
	...
	_, _, errno := syscall.Syscall(syscall.SYS_IOCTL, spi.rc.Fd(), spiIOCWrMaxSpeedHz, uintptr(unsafe.Pointer(&tspeed)))
	if errno != 0 {
		err := syscall.Errno(errno)
		return err
	}
